...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Using cached MNIST IDX gzip files." if cache_exists else "Downloading MNIST IDX gzip files...")
    ...
    for item in history:
        line = f"{item.epoch},{item.loss:.10f},{item.accuracy:.4f},{item.seed}"
        lines.append(line)
        print(
